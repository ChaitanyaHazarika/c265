...
def make_greyscale(input_img):
    ...
    decodeArrayToImage = cv2.imdecode(image_array, cv2.IMREAD_UNCHANGED)
    print('decode values of image: ', decodeArrayToImage)
    convertedGrayImage=  cv2.cvtColor(decodeArrayToImage, cv2.COLOR_RGB2GRAY)
    status, output_img = cv2.imencode('.PNG', convertedGrayImage)
    print("status: ", status)
    return output_img
